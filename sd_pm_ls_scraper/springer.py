...
async def _save_excel_all_pubmed(articles: List[Dict]) -> None:
    if not articles:
        print("     [INFO] No articles to save!")
        return
    ...
    for article in articles:
        ws.append(
            [
                ...
                article.get("link", "null"),
                article.get("authors", "null"),
                article.get("journal_citiation", "null"),
                article.get("citiation_part", "null"),
                article.get("doi", "null"),
                article.get("keyword", "null"),
            ]
        )

    try:
        save_path = os.path.abspath("test_pubmed.xlsx")
        wb.save(save_path)
        print(
            f"    ✅ [INFO] {len(articles)} articles successfully saved to Excel at {save_path}!"
        )
...
class PubMedScraper:
    ...
    async def searching_and_gathering_papers(self, page):
        """Perform a search on PubMed and gather the articles"""
        print("\n📍 Step 1: Connecting to PubMed!")

        try:
            # Navigate to PubMed
            await page.goto(self.url)

            # Truncate very long keywords to prevent issues
            search_term = (
                self.keyword[:100] if len(self.keyword) > 100 else self.keyword
            )
            await page.fill('input[name="term"]', search_term)

            search_button = await page.query_selector(
                "xpath=/html/body/div[2]/main/div[1]/div/form/div/div[1]/div/button"
            )
            if search_button:
            ...
            try:
                # Lower timeout to avoid long waits when no results
                await page.wait_for_selector(
                    ".docsum-content, .results-none-main", timeout=10000
                )
            except Exception as e:
                print(f"No results found or timeout waiting for results: {e}")
                return

            # Check if no results message is present
            no_results = await page.query_selector(".results-none-main")
            if no_results:
                print(f"No results found for keyword: '{self.keyword}'")
                return

            # Continue with results processing
            articles = await page.query_selector_all(".docsum-content")
            ...
            for i, article in enumerate(articles[:5]):
                try:
                    title_link_element = await article.query_selector(".docsum-title")
                    title_link = (
                    ...
                    journal_citiation_element = await article.query_selector(
                        ".docsum-journal-citation"
                    )
                    journal_citiation = (
                        await journal_citiation_element.text_content()
                        if journal_citiation_element
                        else "No journal citation found"
                    )

                    citiation_part_element = await article.query_selector(
                        ".citation-part"
                    )
                    citiation_part = (
                        await citiation_part_element.text_content()
                        if citiation_part_element
                        else "No citation part found"
                    )

                    # Check if DOI exists before splitting
                    doi = "No DOI found"
                    if "doi:" in journal_citiation.lower():
                        doi = journal_citiation.lower().split("doi:")[1].strip()

                    title_element = await article.query_selector(".docsum-title")
                    title = (
                    ...
                    authors_element = await article.query_selector(".docsum-authors")
                    authors = (
                    ...
                    print(f"Journal Citation: {journal_citiation.strip()}")

                    # Handle PMID extraction safely
                    pmid = "No PMID found"
                    if "PMID:" in citiation_part:
                        pmid_parts = citiation_part.strip().split("PMID:")
                        if len(pmid_parts) > 1:
                            pmid = pmid_parts[1]
                    ...
                    article_url = (
                        title_link
                        if title_link and title_link.startswith("http")
                        else f"{self.url.rstrip('/')}{title_link}"
                    )
                    ...
                    self.articles.append(
                        {
                            ...
                            "journal_citiation": journal_citiation.strip(),
                            "citiation_part": pmid,
                            "doi": doi,
                        }
                    )
            ...
            print(f"Error during searching_and_gathering_papers: {e}")
            # No need to raise - just report the error

    async def similarity_finder(self, page):
        import re

        ws = await _load_excel(file_name="test_pubmed.xlsx")

        pattern = re.compile(r"\b" + re.escape(self.keyword) + r"\b", re.IGNORECASE)
        for _row_idx, link in enumerate(
            ws.iter_rows(min_row=2, max_row=5, values_only=True), 2
        ):
            pdf_url = link[1]
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            for _row_idx, author in enumerate(
                ...
                try:
                    scraper = PubMedScraper(keyword=keyword)
                    await scraper.searching_and_gathering_papers(page)
            ...
            if all_articles:
                print(f"\nSaving {len(all_articles)} total articles to Excel...")
                await _save_excel_all_pubmed(all_articles)
                print("No articles found for any keywords.")
